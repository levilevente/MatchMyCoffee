SELECT DISTINCT products.id, products.name, products.description, products.price, products.roast_level, products.acidity_score, bm.name AS brewing_method, pbm.is_optimal AS brewing_method_is_optimal, STRING_AGG(DISTINCT tc.name || ' - ' || t.name, ', ' ORDER BY tc.name || ' - ' || t.name) AS tastes, products.is_blend, STRING_AGG(DISTINCT o.region || ' - ' || o.continent || ' ' || po.percentage, ' + ' ORDER BY o.region || ' - ' || o.continent || ' ' || po.percentage) AS blend FROM products
